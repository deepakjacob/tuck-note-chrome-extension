...
export const CodeMirror = React.forwardRef<
  CodeMirrorRef,
  CodeMirrorProps
>(({ value, onChange, lock, dark }: any, ref) => {
  const divRef = React.useRef<HTMLDivElement>(null);
  const editorRef =
    React.useRef<ReturnType<typeof createCodeMirrorView>>();
  ...
  React.useImperativeHandle(ref, () => ({
    update: (val: string) => {
      ...
      current.setState(
        createCodeMirrorState({ onChange, lock, dark, value: val }),
      );
    },
  }));
  ...
  return (
    <div
      ...
      className={
        className.code + (focus ? ' ' + className.focus : '')
      }
    >
      <div ref={divRef} />
    </div>
